...
class BanditEnv():
	...
	def __init__(self,merged,top_ten,means,standard_deviations=None): 
		...
		self.merged = merged.reset_index()
		self.top_ten = top_ten.reset_index()
        
		return

	def pull_arm(self,pull):

		chosen=self.top_ten['station_id'].iloc[pull]
		arm=pull 
		mu=self.means[arm]
		sigma=self.standard_deviations[arm]
		reward=np.random.choice(self.merged[self.merged['station_id'] == chosen]['total_count'])
